...
public:
    ...
    ManagedArr(T *data, unsigned int new_offset, unsigned int new_len, AllocationType allocation);

    ManagedArr(const ManagedArr& source, unsigned int offset_increase, unsigned int new_len);
...
template <class T>
ManagedArr<T>::ManagedArr():length(0), offset(0){}

template <class T>
ManagedArr<T>::ManagedArr(T *data, unsigned int new_offset, unsigned int new_len, AllocationType allocation) : length(new_len), offset(new_offset)
{
    /*
...
template <class T>
ManagedArr<T>::ManagedArr(const ManagedArr &source, unsigned int offset_increase, unsigned int new_len) : offset(source.offset + offset_increase), length(new_len)
{
    /*
...
template <class T>
ManagedArr<T>::~ManagedArr(){}

template <class T>
inline unsigned int ManagedArr<T>::len() const
{
    return length;
}
